...
# --- Input based on mode ---
if mode == "Drug":
    input_val = st.text_input("Enter Drug name, press Enter then Search:", value=st.session_state["drug_input"]).strip().upper()
else:
    input_val = st.text_input("Enter Gene name,  press Enter then Search:", value=st.session_state["gene_input"]).strip().upper()
